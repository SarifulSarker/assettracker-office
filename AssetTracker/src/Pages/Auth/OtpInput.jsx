...
const OtpInput = ({
  ...
  // ===== Verify OTP mutation =====
  const { mutate: verifyOtpMutate, isLoading: isVerifyOtpLoading } =
    useMutation({
      mutationFn: async (otpValue) => {
        if (actionType === "mfa") {
          return await verifyMfaOtpApi({
            token: tempData?.token,
            otp: otpValue,
          });
        } else {
          return await verifyForgotPasswordOtpApi({
            otp: otpValue,
            email: email || tempData?.identifier,
          });
        }
      },
      onSuccess: (data) => {
  ...
  const { mutate: resendOtpMutate, isLoading: isResendOtpLoading } =
    useMutation({
      mutationFn: async () => {
        return await resendForgotPasswordOtpApi(email || tempData?.identifier);
      },
      onSuccess: () => {
        notifications.show({
          title: "OTP Resent",
